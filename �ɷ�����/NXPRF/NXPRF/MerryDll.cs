    ...
    public class MerryDll : IMerryAllDll
    {
        ...
        public string Run(object[] Command)
        {
            ...
            try
            {
                switch (cmd[1].ToString())
                {
                    ...
                    case "CloseDTM": return CloseDTM().ToString(); // 关闭Power 测试通道

                    case "OpenRx2402DTMCH0": return StartPacketRateTest(SendCommand.OpenRx2402CH00, ReturnValue.OpenRXCH0ReturnValue).ToString(); // 开启RX2402 Power测试通道
                    case "OpenRx2440DTMCH19": return StartPacketRateTest(SendCommand.OpenRx2440CH19, ReturnValue.OpenRXCH0ReturnValue).ToString(); // 开启RX2440 Power测试通道
                    case "OpenRx2480DTMCH39": return StartPacketRateTest(SendCommand.OpenRx2480CH39, ReturnValue.OpenRXCH0ReturnValue).ToString(); // 开启RX2480 Power测试通道
        ...
        string StartPacketRateTest(byte[] SendCommandValue, string ReturnCommandValue)
        {
            try
            {
                ...
                if (!WriteCommand(SendCommand.SetPHYMode2M, ReturnValue.SetPHYModeReturnValue)) return "FalseSetPHYMode2M";
                Thread.Sleep(100);
                if (!WriteCommand(SendCommandValue, ReturnCommandValue)) return "FalseOpenRx";
                Thread.Sleep(200);
                return "True";
            }
            catch (Exception ex)
        ...
        bool CloseDTM()
        {
            try
            {
                bool result = WriteCommand(SendCommand.CloseDTMCH, ReturnValue.CloseDTMCHReturnValue);
                Thread.Sleep(100);
                return result;
            }
            catch
