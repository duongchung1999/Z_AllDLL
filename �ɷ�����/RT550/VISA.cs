...
namespace lvi_Visa
{
    public class VISA
    {
        ...
        public VISA()
        {
            if (!Directory.Exists($"D:\\MerryTestLog\\MerryTest_RT550Log")) Directory.CreateDirectory($"D:\\MerryTestLog\\MerryTest_RT550Log");
        }
        ...
        public string ReadPowerCW(string Channel, string IP)
        {
            if (!Connect(IP)) return "Connent RT550 False";
            WriteCommand($"PORT {this.Port}");
            foreach (var item in CWSetConfig(Channel))
            ...
            if (double.TryParse(Readstr, out double Power))
            {
...
"MEASBLECAPX2 LEICD2M,NA,71764129,37,AOA,20"
        ...
        public string StartTest(String TXP, int timeout, string PATHEDIT)
        {
            try
            {
                foreach (var item in TestCommand(TXP, PATHEDIT))
                {
                    WriteCommand(item);
                }
                bool flag = isIns(timeout);
                WriteCommand("*ins?");
                string str = ReadString();
...
"CONNECT"
        ...
        string[] TestCommand(String TXP, string PATHEDIT)
        {
            string[] Command =
           {
...
$"PATHEDIT 1, CHAN, 39, {PATHEDIT}",//下针，直接连接0-3
$"PATHEDIT 1, CHAN, 78, {PATHEDIT}",//耦合器大概10-15
...
"disconnect"
        ...
        public bool isIns(int PiarCount)
        {
            for (int i = 0; i < PiarCount; i++)
            {
