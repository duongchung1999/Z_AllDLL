...
namespace MerryTest.testitem
{
    ...
    public class Command
    {
        ...
        public List<string> CMDlog = new List<string>();
        FileStream fsDeviceRead;
        AsyncCallback AsyRead;
        public bool StartReadHIDValue(string handle, int Length)
        {
            ...
            if (_createFileHandle == IntPtr.Zero) return false;
            ...
            try
            {
                if (fsDeviceRead != null)
                {
                    AsyRead = (IAsyncResult iResult) =>
                    {
                        ...
                        try
                        {
                            byte[] _arrInputReport = new byte[Length];
                            fsDeviceRead.BeginRead(_arrInputReport, 0, Length, AsyRead, _arrInputReport);
                        }
                        catch { }

                        lock (CMDlog)
                        {
                            string str = Encoding.ASCII.GetString(arrBuff).Replace("\0", "").ToUpper();
                            CMDlog.Add(str);
