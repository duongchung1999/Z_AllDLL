...
namespace ANC
{
    ...
    public class INIClass
    {
        ...
        public static string GetValue(string section, string key, string _path)
        {
        ...
        public static long SetValue(string section, string Key, string value, string _path)
        {
            return WritePrivateProfileString(section, Key, value, _path);
        }
        [DllImport("kernel32.dll", CharSet = CharSet.Auto)]
        ...
        /// <returns></returns>
        public static Dictionary<string, string> GetSection(string category, string iniFile)
        {
            byte[] buffer = new byte[4096];
            GetPrivateProfileSection(category, buffer, 4096, iniFile);
        ...
        public static Dictionary<string, string> GetSection(string category)
        {

            return GetSection(category, _path);
        }
        ...
        /// <returns></returns>
        public bool WriteIni(string section, string Key, string Value, string path)
        {
            try
            {
                if (!File.Exists(path))
                ...
                WritePrivateProfileString(section, Encoding.Default.GetBytes(Key), Encoding.UTF8.GetBytes(Value), path);
                return true;
            }
            catch (Exception ex)
            {
