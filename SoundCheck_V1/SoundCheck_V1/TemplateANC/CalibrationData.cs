...
namespace SoundCheck_V1.TemplateANC
{
    public class CalibrationData
    {
        ...
        public string ANCAdjust_ForCurvesFB(
            ...
            string FilePath, out bool NGCurvesIf,
            ref double L_Gain, ref double R_Gain
           )
        {
            NGCurvesIf = false;
            double LastGainL = L_Gain;
            double LastGainR = R_Gain;
            try
            {
                CurvesInfo.info info = new CurvesInfo.info();
                //读取数据
                ReadFB_SCData(FilePath, true, ref info);

                //补充缺失的limit
                SupplementLimit(ref info);
                infos[$"{LastGainL}&&{LastGainR}"] = info;
                //拿到校准值
                //检测曲线当前状态
                //string CheckCurvesResult = _CheckCurves(info, new Dictionary<string, double[]>(), new Dictionary<string, double[]>());
                //NGCurvesIf = (CheckCurvesResult == "Rising Limit False");

                GetGainRegion(_Parameter.nb_FBGainMax, _Parameter.nb_FBGainMin,
                    L_FB_CurveGianCardinal[L_Gain], R_FB_CurveGianCardinal[R_Gain],
                ...
                string logStr = "";
                StringBuilder adjustLog = new StringBuilder();
                adjustLog.AppendLine($"左耳目标：\r\n");

                foreach (var item in L_PassCurves)
                    logStr += $"{item.Key}，";
                adjustLog.AppendLine($"嵌入Limit L FB Pass数量:{L_PassCurves.Count} // :{logStr}\r\n");
                logStr = "";
                foreach (var item in R_PassCurves)
                    logStr += $"{item.Key}，";
                adjustLog.AppendLine($"嵌入Limit R FB Pass数量:{R_PassCurves.Count} // :{logStr}\r\n");
                ...
                //    RightGain = double.Parse(Next[1]).Round(1);
                //}
                //if (_Parameter.cb_ShowCailbration && lastGain != NextDiffCurves.CurveName)
                //{
                //    Task.Run(() =>
                //    {
                //        _CurvesData L_Curves = L_NotScreenAllCurves[double.Parse(Next[0]).Round(1)];
        ...
        public string ANCAdjust_ForCurvesFF(
         ANC_Calibration_Parameter _Parameter,
         string SequenceDic, out bool NGCurvesIf,
         ref double L_Gain, ref double R_Gain
        )
        {
            NGCurvesIf = false;
            ...
            try
            {
                CurvesInfo.info info = new CurvesInfo.info();
                //读取数据
                ReadFB_SCData(SequenceDic, false, ref info);
                //补充缺失的limit
                SupplementLimit(ref info);
                infos[$"{LastGainL}&&{LastGainR}"] = info;
                //获取所有的Gain
                ...
                ScreenFFMinSingle(_Parameter, L_PassCurves, R_PassCurves, out _, out _);
                return "True";
            ...
            )
        {
            ...
            if (_Parameter.gb_FFMinType.isRegion())
            {
                ...
                int[] L_Min = ScreenApproachFigure(L_RegionMicFreq.Values.ToArray(), _Parameter.nb_FFMinTarget, 10);
                ...
                int[] R_Min = ScreenApproachFigure(R_RegionMicFreq.Values.ToArray(), _Parameter.nb_FFMinTarget, 10);
        ...
        public void GetGainRegion(double Up, double Down,
            Dictionary<double, double[]> L_data, Dictionary<double, double[]> R_data,
            out Dictionary<double, double[]> L_Cardinal, out Dictionary<double, double[]> R_Cardinal)
        {
            ...
            out Dictionary<double, _CurvesData> R_SelectCurves)
        {
            ...
            {
                MessageBox.Show($"=> 0008 筛选靠近目标值报错了 目标值：\r\n“{ex.StackTrace}”", "算法异常");
            }
            try
            {
            ...
            {
                MessageBox.Show($"=> 0009 筛选靠近目标值报错了 目标值： \r\n“{ex.StackTrace}”", "算法异常");
            }
        }
        ...
        void ReadFB_SCData(string FilePath, bool isFB, ref CurvesInfo.info info)
        {
        ...
        int[] ScreenApproachFigure(double[] ListFigure, double ApproachFigure, int Amount)
        {
