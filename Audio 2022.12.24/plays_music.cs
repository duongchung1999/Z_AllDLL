...
namespace WindowsFormsApplication1.FunctionalTest.Interface
{
    class SoundRecord
    {
        ...
        public static void aa(string device)
        {
            name = device;
        }
        /// <summary>
        ...
        /// 开始录音
        /// </summary>
        public bool RecStart(string _device)
        {
            ...
            if (messagebox("录音时说123，Mute on后说321"))
            {
                RecStop();
                return pl.PlayTest(_device, _handle, @".\Music\rec.wav", false, "播放录音");
            }
            else
            {
                RecStop();
