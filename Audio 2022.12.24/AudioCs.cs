...
namespace MerryTestFramework
{
    ...
    public class Audio
    {
        ...
        /// <returns></returns>
        public static bool PlayTest(string DeviceName, IntPtr handle, string music, bool restart, string message)
        {
            if (!playmusic(DeviceName, handle, music, restart)) return false;
            // bool flag = messagebox(message);
            bool flag = false; 
            fMessage msg = new fMessage(message, 500);
        ...
        public static bool MusicPlay(string music, string message)
        {
      
            //_stream = Bass.BASS_StreamCreateFile(music, 0, 0, BASSFlag.BASS_SAMPLE_LOOP);
            //if (_stream == 0) return false;
            //if (!Bass.BASS_ChannelPlay(_stream, true)) return false;
            plays.StartMusic(string.Concat(path, $@"\\Music\\{music}.wav"));
            fMessage mes = new fMessage(message, 500);
        ...
        public static bool RecordPlayTest(string message, string message2)
        {
            ...
            //MessageBox.Show("对着麦克风说话来录音，然后按OK - Nói vào Mic để ghi âm, sau đó nhấn OK");
            fMessage mes = new fMessage(message, 500);
            mes.StartPosition = FormStartPosition.CenterParent;
            if (mes.ShowDialog() == DialogResult.Yes)
            {
                ...
                fMessage mes1 = new fMessage(message2, 300);
        ...
        /// <param name="message"></param>
        /// <returns></returns>
        public static bool RecordTest(String DeviceName, IntPtr handle, string message, string recMessage )
        {
            ...
            if (Record1(1, DeviceName, message))
            {
                if (PlayTest(DeviceName, handle, @".\Music\rec.wav", false, recMessage)) return true;
            }
        ...
        public static bool Sidetone(string msg)
        {
            ...
            //bool flag = messagebox(msg);
            fMessage msg1 = new fMessage(msg, 500);
            msg1.StartPosition = FormStartPosition.CenterParent;
            bool flag = false;
            if (!(msg1.ShowDialog() == DialogResult.Yes))
            {
                flag = false;
            }
            else
        ...
        private static List<int> AllIndexOfAudioCard(Int32 type, String DeviceName)
        {
            BASS_DEVICEINFO[] DeviceInfos = null;
            System.Text.RegularExpressions.Regex MatchItem = new System.Text.RegularExpressions.Regex(DeviceName);
        ...
        /// <param name="restart"></param>
        /// <returns></returns>
        public static bool playmusic(String DeviceName, IntPtr handle, string music, bool restart)
        {
        ...
        public static void playmusic2(String DeviceName, IntPtr handle, string music, bool restart)
        {
            
            List<int> PlayerIndex = AllIndexOfAudioCard(0, DeviceName);
            if (PlayerIndex == null) return ;
            foreach(var item in PlayerIndex)
            {
                Bass.BASS_Init(item, 44100, BASSInit.BASS_DEVICE_DEFAULT, handle);
                _stream = Bass.BASS_StreamCreateFile(music, 0, 0, BASSFlag.BASS_SAMPLE_LOOP);
                if (_stream == 0) return ;
                Bass.BASS_ChannelPlay(_stream, restart);
            }
        }

        public static bool playmusic3(String DeviceName, IntPtr handle, string music, bool restart)
        {
            int PlayerIndex = GetIndexOfAudioCard(0, DeviceName);
            for (int i=0; i < 20; i++)
            {
        ...
        private static void Record(int type, string DeviceName, string message)
        {
            var recorder = new SoundRecord();
            int PlayerIndex = GetIndexOfAudioCard(type, DeviceName);
            string wavfile = @".\Music\rec.wav";
        ...
        private static bool Record1(int type, string DeviceName, string message)
        {
            var recorder = new SoundRecord();
            int PlayerIndex = GetIndexOfAudioCard(type, DeviceName);
            string wavfile = @".\Music\rec.wav";
