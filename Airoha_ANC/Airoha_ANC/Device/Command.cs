...
namespace MerryTest.testitem
{
    ...
    public class Command
    {
        ...
        /// <param name="intPtr">句柄通道指针</param>
        /// <returns>指令是否下成功</returns>
        public bool WriteReturns(string command, int lenght, string readdata, string Indexes, string path, IntPtr handle, out string[] str)
        {
            str = new string[lenght];
            ReturnValue = "False";
            ALLReturnValue = "False";
            //  if (path == "" || intPtr == IntPtr.Zero) return false;
            Thread.Sleep(50);
            ...
            try
            {
                ...
                {
                    WriteSend(command, lenght, handle);
                    if (endflag)
                    {
                        if (readflag)
                        {
