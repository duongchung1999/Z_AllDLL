...
namespace Airoha.AdjustANC
{
    public class AdjustClass2
    {
           ...
           )
        {
            ...
            try
            {
                string last = $"{LeftGain}&{RightGain}";
                ...
                if (CheckCurvesResult.Contains("Adjust True")) return "Adjust True";
                ...
                if (ShowFroms)
                {
                    Task.Run(() =>
                    {
                        ...
                        CurvesForms forms = new CurvesForms(5, "平衡优先");
                        forms.lb_Gain.Text = $"Last: {last} Next:{NextDiffCurves.CurveName}";
        ...
        string CheckCurves(CurvesInfo.info info, double MaxDiff)
        {
            ...
            if (!ifCorves(info.L_Curves, info.L_Curves_Uplimit, info.L_Curves_Lowlimit)
                || !ifCorves(info.R_Curves, info.R_Curves_Uplimit, info.R_Curves_Lowlimit))
                return "False";
    ...
    public static class _class2
    {
        ...
        public static string dicToTarget(this Dictionary<string, double[]> dic)
        {
            string str = "";
            foreach (var item in dic)
            {
                string str1 = "{" + item.Key + ":" + item.Value[0] + ":" + item.Value[1] + "}";
                str += str1;
            }
            return str;
        }
