...
namespace Airoha.AdjustANC
{
    public partial class CurvesForms : Form
    {
        ...
        public void AddDiffCurves(_CurvesData data)
        {
        ...
        private void CurvesDiffTimer_Tick(object sender, EventArgs e)
        {
            ...
            lock (curvesDiffDatas)
            {
                ...
                foreach (var data in curvesDiffDatas)
                {
                    ...
                    for (int i = 0; i < Diff.Series.Count; i++)
                    {
                        if (Diff.Series[i].Name.Contains(data.CurveName))
                            SeriesExist = true;
                    }
                    if (SeriesExist)
                        continue;
