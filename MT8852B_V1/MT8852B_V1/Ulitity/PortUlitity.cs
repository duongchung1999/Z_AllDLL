...
namespace VISAInstrument.Port
{
    ...
    class PortUltility
    {
        ...
        public static bool OpenIPAddress(string address, out string fullAddress)
        {
            ...
            try
            {
                using (GpibSession session = new GpibSession(addressTemp))
                {
                    result = true;
                }
            }
        ...
        public static bool OpenRS232Address(string address, out string fullAddress)
        {
            bool result = false;
            string addressTemp = address;
            fullAddress = addressTemp;
            try
            {
                using (SerialSession session = new SerialSession(addressTemp))
                {
                    result = true;
                }
            }
