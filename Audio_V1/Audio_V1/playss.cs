...
namespace WindowsFormsApplication1.FunctionalTest.Interface
{
    class SoundRecordss
    {
        ...
        /// 构造函数,设定录音设备,设定录音格式.
        /// </summary>
        public SoundRecordss(string DeviceName)
        {
            Device = DeviceName;
            // 初始化音频捕捉设备
            InitCaptureDevice();
        ...
        public static void GetDeviceName(string device)
        {
            Device = device;
        }
        /// <summary>
        ...
        /// 开始录音
        /// </summary>
        public string Record(string path, string message)
        {
            try
            {
                if (mCapDev == null) return "False";
                // 创建录音文件
                CreateSoundFile();
                // 创建一个录音缓冲区，并开始录音
                CreateCaptureBuffer();
                ...
                messagebox(message);
                RecStop();
                return MerryTestFramework.Audio.PlayTest(Device, IntPtr.Zero, path, true, "播放录音");
            }
            catch(Exception ex)
            {
                RecStop();
        ...
        private bool InitCaptureDevice()
        {
            ...
            {
                if (devices[i].ToString().Contains(Device))
                {
                    deviceGuid = devices[i].DriverGuid;
                    break;
                }
            }
